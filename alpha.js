...
async function startalpha() {
    bot.on('callback_query', async (alpha) => {
        ...
        const username = user.username ? user.username : "rulzxyz";
        ...
        try {
            switch (action[0]) {
                ...
                case "owner": {
                    ...
                    reply(`LORD IRULZXD [${OWNER_NAME}](${OWNER[0]}) 👑`)
                }
                break
                case "ytmp3": {
                    if (!args[2]) return reply(`Kirim perintah:\n/ytmp3 link youtube\n\nContoh penggunaan:\n/ytmp3 https://youtu.be/XpVsobekyh8?si=BK87icU1E6V0u8L9`)
                ...
                break
                case "ytmp4": {
                    if (!args[2]) return reply(`Kirim perintah:\n/ytmp4 link youtube\n\nContoh penggunaan:\n/ytmp4 https://youtu.be/XpVsobekyh8?si=BK87icU1E6V0u8L9`)
                    if (!isUrl(args[2])) return reply(`Kirim perintah:\n/ytmp4 link youtube\n\nContoh penggunaan:\n/ytmp4 https://youtu.be/XpVsobekyh8?si=BK87icU1E6V0u8L9`)
                    if (!args[2].includes('youtu.be') && !args[2].includes('youtube.com')) return reply(`Kirim perintah:\n/ytmp4 link youtube\n\nContoh penggunaan:\n/ytmp4 https://youtu.be/XpVsobekyh8?si=BK87icU1E6V0u8L9`)
                    reply(lang.wait)
                    await alpha.deleteMessage()
    ...
    bot.command('help', async (alpha) => {
        ...
        await alpha.reply(lang.first_chat(BOT_NAME, user.full_name), {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{
                        text: 'MY GROUP IRULZPEDIA',
                        url: "https://t.me/irulzpedia"
                    }, {
                        text: 'OWNER 😁',
                        url: OWNER[0]
                    }]
